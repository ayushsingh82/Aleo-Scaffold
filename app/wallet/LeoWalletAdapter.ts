...
export class LeoWalletAdapter extends BaseMessageSignerWalletAdapter {
    ...
    constructor({ appName = 'sample', isMobile = false, mobileWebviewUrl} : LeoWalletAdapterConfig = {}) {
        ...
        this._checkWalletAvailability(); // Check immediately on init

        if (this._readyState !== WalletReadyState.Unsupported) {
            scopePollingDetectionStrategy(() => {
                this._checkWalletAvailability(); // Also check via polling
                return this._readyState === WalletReadyState.Installed;
            });
        }
    }

    private async _checkWalletAvailability() {
        if (window?.leoWallet || window?.leo) {
            ...
            if (window?.leoWallet && window?.leoWallet.isAvailable) {
                await window?.leoWallet.isAvailable();
            }
        } else {
    ...
    async connect(decryptPermission: DecryptPermission, network: WalletAdapterNetwork, programs?: string[]): Promise<void> {
        try {
            ...
            // Re-check availability right before connecting
            await this._checkWalletAvailability();
            ...
            // isAvailable check is now part of _checkWalletAvailability
